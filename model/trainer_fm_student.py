...
class DiffFSTrainer:
    def __init__(self, args):
        ...
        if self.args.z_sem_flowmatch:
            ...
            fm_ckpt = torch.load(fm_ckpt_path, map_location=self.args.device)
            self.flowmatch_z_sem.load_state_dict(fm_ckpt['fm_module'], strict=False)
            if 'stats_mean' in fm_ckpt and 'stats_std' in fm_ckpt:
                self.flowmatch_z_sem.set_normalization_stats(fm_ckpt['stats_mean'], fm_ckpt['stats_std'])
                print("FlowMatchingModule stats loaded from checkpoint.")
            self.flowmatch_z_sem.to(self.args.device)

        if self.args.enable_spnet:
            self.model_map = SPNet().to(args.device)

        # prepare train
        self.diffae_A, self.diffae_B, self.diffae_ghost, self.model_map_stu, self.conds_std, self.conds_mean = self.prepare_train(model_diffae, self.model_map)

        if self.args.enable_spnet:
            self.model_map_teacher = self.model_map
                
        self.netD = D_NLayersMulti(input_nc=3, ndf=64, n_layers=3, num_D=1).to(self.args.device)
        self.optim_D = optim.Adam(self.netD.parameters(), lr=self.args.lr / 2, betas=(0.5, 0.999)) # lr 
        ...
        if torch.cuda.is_available() and torch.cuda.device_count() > 1:
            ...
            if self.args.enable_spnet:
                ...
                self.model_map_teacher = nn.DataParallel(self.model_map_teacher)
        ...
        #todo. StepLR for optimizers 
        step_size = getattr(self.args, 'lr_step', 50)
        gamma = getattr(self.args, 'lr_gamma', 0.5)
        self.scheduler_diffae_ghost = optim.lr_scheduler.StepLR(self.optim_diffae_ghost, step_size=step_size, gamma=gamma)
    ...
    def load_checkpoint_for_continue(self, start_epoch):
        ...
        load_state_dict_flexible(self.flowmatch_z_sem, fm_ckpt['fm_module'])
        if 'stats_mean' in fm_ckpt and 'stats_std' in fm_ckpt:
            self.flowmatch_z_sem.set_normalization_stats(fm_ckpt['stats_mean'], fm_ckpt['stats_std'])
        else:
        ...
        load_state_dict_flexible(self.diffae_ghost, ckpt['diffae_ghost'])
        ...
        if 'optim_diffae_ghost' in ckpt:
            self.optim_diffae_ghost.load_state_dict(ckpt['optim_diffae_ghost'])
    ...
    def prepare_train(self, model_diffae, model_map=None):
        ...
        teacher_path = "./prepared_teacher_ckpt/epoch_30.pt"
        print(f"Loading diffAE_B teacher model from {teacher_path} for initialization...")

        teacher_ckpt = torch.load(teacher_path, map_location='cpu')
        state_dict_B = teacher_ckpt['diffae_B']
        new_state_dict_B = {}
        for k, v in state_dict_B.items():
            name = k[7:] if k.startswith('module.') else k
            new_state_dict_B[name] = v

        diffae_B.load_state_dict(new_state_dict_B, strict=True)
        diffae_B = diffae_B.to(self.args.device)
        diffae_B.eval()
        diffae_B.requires_grad_(False)

        if model_map is not None and 'model_map' in teacher_ckpt:
            print("Loading teacher's map_net weights...")
            state_dict = teacher_ckpt['model_map']
            new_state_dict = {}
            for k, v in state_dict.items():
                name = k[7:] if k.startswith('module.') else k
                new_state_dict[name] = v
            
            model_map.load_state_dict(new_state_dict, strict=True)
            model_map.eval()
            model_map.requires_grad_(False)
            print("Teacher's map_net weights loaded.")

            model_map_stu = copy.deepcopy(model_map)
            model_map_stu.train()
            model_map_stu.requires_grad_(True)
            print("Student map_net initialized for training.")

        diffae_ghost = copy.deepcopy(model_diffae.ema_model)
        diffae_ghost = diffae_ghost.to(self.args.device)
        diffae_ghost.train()
        diffae_ghost.requires_grad_(True)

        # latent ddim z statistics
        conds_std = model_diffae.conds_std  
        conds_mean = model_diffae.conds_mean

        return diffae_A, diffae_B, diffae_ghost, model_map_stu, conds_std, conds_mean
    ...
    def compute_loss(self, img_A, img_B, x0_pred_style_B):
        ...
        l_l1 = self.compute_recon_l1_loss(img_B, x0_pred_style_B)
        loss += l_l1
        loss_dict['L1'] = l_l1.item()
        
        # loss_content_withteacher = self.structure_loss_fn(x0_pred_style_B, img_B) * self.content_weight
        loss_content_withsource = self.structure_loss_fn(x0_pred_style_B, img_A) * self.content_weight
        # loss_content = (loss_content_withteacher + loss_content_withsource) * 0.5
        loss_content = loss_content_withsource
        loss += loss_content
    ...
    def save_image(self, i, epoch, img_A, img_B, A_pred_B_student, A_pred_B_teacher):
        imgs = torch.cat([img_A, img_B, A_pred_B_student, A_pred_B_teacher], dim=0)
    ...
    def save_model(self, epoch):
        ...
        if self.args.enable_spnet:
            content = {
                'iter': epoch+1,
                'diffae_ghost': self.diffae_ghost.state_dict(),
                'model_map': self.model_map.state_dict(),
                'optim_diffae_ghost': self.optim_diffae_ghost.state_dict(),
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.args.n_iter):
            ...
            if epoch in self.content_decay_epochs:
            ...
            for i, data in tqdm(enumerate(self.dataloader), total=len(self.dataloader)):
                img_A = data['A'].to(self.args.device)  # source domain image
                img_B = data['B'].to(self.args.device)  # target domain image             
                filename = data['filename'][0]

                # encode image for z_sem.
                ...
                if self.args.z_sem_flowmatch:
                    z_sem_B_sampled = self.flowmatch_z_sem.transfer(z_sem_A)
                ...
                xt_A_teacher = xt_A.clone()

                for j in backward_indices:
                    t = torch.tensor([j]*len(img_A), device=self.args.device) 
                    with torch.no_grad():
                        if self.args.enable_spnet:
                            map_teacher = self.model_map_teacher(img_A, t)
                            map_teacher = torch.sigmoid(map_teacher)
                            xt_A_teacher = xt_A_teacher + 0.1*map_teacher
                            # xt_A = xt_A + self.args.lambda_map*map_style

                        out_style_teacher = self.train_samp_back.ddim_sample(self.diffae_B,
                                                                    xt_A_teacher,
                                                                    t,
                                                                    model_kwargs={'cond': z_sem_B})
                        x0_pred_B_teacher = out_style_teacher['pred_xstart'] 
                        xt_A_teacher = out_style_teacher['sample'].detach().clone()

                    if self.args.enable_spnet:
                        map_style = self.model_map_stu(img_A, t)
                        map_style = torch.sigmoid(map_style)
                        xt_A = xt_A + 0.1*map_style
                        # xt_A = xt_A + self.args.lambda_map*map_style

                    out_style_student = self.train_samp_back.ddim_sample(self.diffae_ghost,
                                                                xt_A,
                                                                t,
                                                                model_kwargs={'cond': z_sem_B_sampled})
                    x0_pred_B_student = out_style_student['pred_xstart'] 
                    
                    total_loss, loss_dict = self.compute_loss(img_A, x0_pred_B_teacher, x0_pred_B_student)

                    #================Discriminator Losses Added Begin================#
                    self.set_requires_grad(self.netD, True)
                    self.set_requires_grad([self.diffae_ghost, self.model_map_stu, self.condition_adapter], False)

                    self.optim_D.zero_grad()
                    pred_real = self.netD(img_B)
                    loss_D_real = self.criterionGAN(pred_real, True).mean()
                    pred_fake = self.netD(x0_pred_B_student.detach()) 
                    ...
                    self.set_requires_grad([self.diffae_ghost, self.model_map_stu, self.condition_adapter], True)
                    
                    pred_fake_G = self.netD(x0_pred_B_student)
                    ...
                    self.optim_diffae_ghost.zero_grad()
                    self.optim_map.zero_grad()
                
                    total_loss.backward()

                    self.optim_diffae_ghost.step()
                    self.optim_map.step()

                    xt_A = out_style_student['sample'].detach().clone()
                ...
                # save images
                if (i+1)%10==0:
                    self.save_image(i, epoch, img_A, img_B, x0_pred_B_student, x0_pred_B_teacher)
