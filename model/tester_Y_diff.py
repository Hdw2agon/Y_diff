...
class DiffFSTester:
    def __init__(self, args):
        ...
        self.diffae_ghost = copy.deepcopy(model_diffae.ema_model)
        self.diffae_ghost = self.diffae_ghost.to(self.args.device)
        self.diffae_ghost.eval()
        self.diffae_ghost.requires_grad_(False)
        ...
        if self.args.z_sem_flowmatch:
            self.flowmatch_z_sem = FlowMatchingModule(dim=512, hidden_dim=2048, num_layers=8, use_ot=True, device=self.args.device).to(self.args.device)
        ...
        elif 'er004' in self.args.dataset_path.lower():
    ...
    def infer_image(self):
        ...
        for i, data in tqdm(enumerate(self.dataloader), total=len(self.dataloader)):
            ...
            if self.args.z_sem_flowmatch:
                z_sem_fm = self.flowmatch_z_sem.transfer(z_sem_A)
            ...
            with torch.no_grad():
                ...
                for j in reverse_indices:
                    ...
                    out = self.infer_samp_back.ddim_sample(self.diffae_ghost,
                                                            xt_B,
                                                            t,
                                                            model_kwargs={
                                                                'cond': z_sem_fm if self.args.z_sem_flowmatch else z_sem_A
                                                            })
                ...
                if hasattr(self, 'infer_only') and self.infer_only:
                    save_dir = os.path.join(self.args.work_dir, 'infer_only')
                else:
    ...
    def infer_image_all(self):
        ...
        print("begin load state dict diffae_ghost")
        self.diffae_ghost.load_state_dict(ckpt['diffae_ghost'])
        self.diffae_ghost = self.diffae_ghost.to(self.args.device)
        print("end load state dict diffae_ghost")
    ...
    def infer_image_all_multiGPU(self):
        ...
        print("begin load state dict diffae_ghost")
        new_diffae_ghost_state_dict = {}
        original_diffae_ghost_state_dict = ckpt['diffae_ghost']
        for k, v in original_diffae_ghost_state_dict.items():
            if k.startswith('module.'):
                new_key = k[len('module.'):]
                new_diffae_ghost_state_dict[new_key] = v
            else:
                new_diffae_ghost_state_dict[k] = v

        self.diffae_ghost.load_state_dict(new_diffae_ghost_state_dict)
        self.diffae_ghost = self.diffae_ghost.to(self.args.device)
        print("end load state dict diffae_ghost")
